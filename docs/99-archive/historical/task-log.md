...
| P5 | 侧边栏样式重做 | #35/#36 两次尝试均 ❌，需重新设计方案 |
| P6 | 上线实施 | Cloudflare Tunnel + Access 配置，见 `deploy.md` |

### P5.blog-auto-sync 📝 ✅

**扩展 sync-projects.js 自动扫描 blog/ 目录**:
- [x] 监听 blog/ 目录，识别 frontmatter（title/date/tags）
- [x] 自动生成 `blog/index.md` 文章列表
- [x] 自动生成 `blog/tags.md` 标签索引
- [x] 自动更新 `.vitepress/config.ts` 侧边栏博客条目
- [x] watch.js 增加 blog/ 目录监听

### P5.theme-toggle 🌗 ✅

**增加白色主题**:
- [x] VitePress 端启用 `appearance: true`，新增浅色 CSS 变量
- [x] 三个独立 HTML 页面添加 light class 切换
- [x] localStorage 记忆 + 系统偏好检测
- [x] 按钮统一位于标题栏右侧

### P5.sidebar-dynamic 📑 ✅

**侧边栏随文章自动更新**:
- [x] sync-projects.js 增加 `genSidebarBlock()` 函数
- [x] config.ts 添加 `SIDEBAR:BLOG` 标记，自动替换
- [x] 文章增删自动反映到侧边栏

### P5.sidebar-deepseek 🎯 ✅

**仿 DeepSeek 官方侧边栏，博客文章分两组可折叠**:

- [x] sync-projects.js `genSidebarBlock()` 按标签分类文章
  - buildingTags: VitePress/Obsidian/前端/前端架构/前端工程化/Vue/Node.js/CSS/JavaScript/架构设计
  - carTags: ROS/智能车/控制算法/嵌入式
- [x] 生成两组 `collapsed: false` 的 sidebar 分组：「博客搭建」(4篇) + 「智能车」(2篇)
- [x] 修复 `parseFrontmatter` 支持 YAML 多行列表格式（之前「智能车赛道初体验」的多行 tags 解析为空数组）
- [x] custom.css 改为 DeepSeek 风格：
  - 显示 caret 图标，旋转动画（-90deg when collapsed）
  - 分组标题加粗加大、子项缩进
  - 分组间分隔线（border-top）
  - sidebar 用 `--sidebar-bg` 暗色背景，与内容区视觉区分
  - sidebar 220px，布局 1440px，正文 960px
- [x] 分离 `:root` 浅色 / `html.dark` 深色变量，添加 `--sidebar-bg` 两种色值

### P5.sidebar-width ✅

**加宽侧边栏 + 文本拉近边缘**:
- [x] sidebar 220→260px，布局 1440→1520px
- [x] 添加文字溢出截断（overflow/text-overflow/white-space）
- [ ] 文字左靠边（减 padding/margin）→ ❌ 未达预期

### P5.sidebar-wrap ✅

**多行文本换行**:
- [x] 移除单行截断，改为 `word-break: break-word` + 自然换行
- [x] 折叠标题 `align-items: flex-start` 适配多行
- [x] 子项 line-height 1.4 保持紧凑

### P5.sidebar-caret 🔧

**智能车折叠符号待修复**:
- [ ] caret 图标与标题垂直对齐问题（flex-start → center）

### P5.sidebar-style-failed ❌

**多轮侧边栏样式调整失败**:

| 轮次 | 尝试 | 结果 |
|------|------|------|
| 1 | 加 border-right 分隔线 | ❌ "分隔栏位置不好" |
| 2 | 仿 DeepSeek 风格（不同背景色替代边框） | ❌ "侧边栏略宽、布局不够靠边" |
| 3 | 缩窄 220px、扩宽布局至 1440px、加宽正文至 960px | ❌ "跟文章没有明显分界" |

**教训**: 侧边栏样式需要重新设计，当前方案未达预期。后续应:
- 参考 DeepSeek 官方文档做更精细的间距和配色
- 在 content 与 sidebar 之间加视觉过渡（内阴影或微妙渐变）
- 侧边栏激活态、悬浮态统一设计
