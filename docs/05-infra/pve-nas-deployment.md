...
当前仓库的 GitHub Actions 会验证代码并构建 `linux/amd64` 镜像；远端 NAS 发布仍由操作者在 VM 内执行。当前已完成 CI 与远端部署，后续发布应按版本 tag 和发布记录执行：
...
CI 与远端部署已完成。后续如果某次 Ubuntu/Windows verify 失败，`nas-image` 会被跳过；该次提交不得作为生产发布依据，需修复后重新运行完整 CI。
