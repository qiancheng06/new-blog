...
| P4 | Workspace Agent | Modernize Workspace entrypoint so the served app is the only primary UI path; retire or wrap legacy standalone HTML assets behind the dev-server workflow. | `apps/workspace/**`, `docs/01-workspace/**`, `README.md` | `http://127.0.0.1:5173/` remains the documented browser entrypoint; no root or file-protocol entrypoint is required. |
| P5 | Product/Runtime Agent | Define permissioned real-mode evaluation for DeepSeek quality and Telegram end-to-end behavior without adding it to the default AI gate. | `docs/02-persona/**`, `docs/05-infra/**`, `docs/07-product/**` | A human-run checklist exists for real network credentials, model quality, Telegram messaging, and rollback notes. |
