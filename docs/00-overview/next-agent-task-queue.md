...
| P1 | Workspace Agent | Improve local startup UX: document and, if needed, automate the two-process dev flow for `npm.cmd run dev` plus `npm.cmd run dev:backend:mock`. | `apps/workspace/start-blog.bat`, `apps/workspace/index.html`, `docs/01-workspace/**`, `README.md` | Frontend loads at `http://127.0.0.1:5173/`; mock API status loads at `http://127.0.0.1:3001/api/status`. |
