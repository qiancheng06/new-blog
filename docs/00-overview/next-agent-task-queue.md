...
| P12 | Product Agent | Define and implement the next Memory management step: governed edit/archive actions or Daily Summary write-back, depending on real-mode evaluation results. | `docs/03-memory/**`, `docs/04-application/**`, `docs/07-product/**`, `apps/persona/src/**`, `apps/workspace/.vitepress/theme/**` | Scope is split before implementation; write actions go through Events/Application, not direct Workspace-to-Memory mutation. |
