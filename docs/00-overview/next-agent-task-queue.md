...
| P6 | Runtime Agent | Add a human-triggered real-mode diagnostics command that prints readiness status without leaking secrets or calling providers. | `apps/persona/src/infra/**`, `apps/persona/src/application/**`, `docs/05-infra/**` | The command is no-network, redacts secrets, and helps humans prepare for `real-mode-evaluation.md`. |
