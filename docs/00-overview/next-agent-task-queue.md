...
| P7 | Memory/Governance Agent | Define and implement a safe test-data cleanup path for real-mode evaluation events and Memory patches. | `apps/persona/src/domain/memory/**`, `apps/persona/src/domain/event/**`, `docs/03-memory/**`, `docs/07-product/**` | Human real-mode tests can be labeled, inspected, and cleaned up without deleting unrelated Memory. |
