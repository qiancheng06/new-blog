...
| P9 | Product/Runtime Agent | Execute the human-run real-mode evaluation and record the result packet for DeepSeek, Telegram, Workspace real-backend behavior, rollback, and cleanup evidence. | `docs/07-product/**`, `docs/05-infra/**`, `docs/03-memory/**` | Preflight is recorded in `docs/07-product/evaluation-results/2026-07-04-p9-preflight.md`; first DeepSeek attempt is recorded in `docs/07-product/evaluation-results/2026-07-04-p9-deepseek-auth-failed.md` and failed on the old configured key; full acceptance still requires a rerun with a valid local key, `docs/07-product/real-mode-evaluation.md` to be followed, `docs/07-product/real-mode-evaluation-result-template.md` to be filled into a dated result note, cleanup dry-run output to be recorded, and any real-mode failures to become bounded follow-up tasks. |
...
| P11 | Workspace Agent | Add a served Workspace Memory/Profile panel that consumes the read-only Application APIs and does not use legacy HTML or direct SQLite access. | `apps/workspace/.vitepress/theme/**`, `docs/01-workspace/**`, `docs/07-product/**` | `npm.cmd run check:workspace` and `npm.cmd run verify:local` pass; panel has offline/empty states and reads `http://127.0.0.1:3001/api/memory/profile` through a shared Persona API client. |
