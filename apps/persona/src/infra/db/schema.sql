...
CREATE TABLE IF NOT EXISTS calendar_events (
  ...
  reminder TEXT NOT NULL DEFAULT '{"kind":"none"}',
  version INTEGER NOT NULL DEFAULT 1,
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  updated_at TEXT NOT NULL DEFAULT (datetime('now')),
  deleted_at TEXT,
...
-- Mobile credentials are intentionally separate from browser sessions. Only
-- SHA-256 hashes of pairing, refresh, and access tokens are persisted.
...
CREATE INDEX IF NOT EXISTS idx_mobile_pairing_codes_expiry
  ON mobile_pairing_codes(expires_at, used_at);
...
CREATE INDEX IF NOT EXISTS idx_mobile_devices_active ON mobile_devices(revoked_at, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_mobile_refresh_tokens_device ON mobile_refresh_tokens(device_id, revoked_at);
