...
const workingStateText = [
  "Mode: S1 (companion-first)",
  "Current project: Persona OS MVP",
  "Active topics: architecture, delivery",
  "Current questions:",
  "- What closes the current MVP?",
].join("\n")
...
const prompts = buildPrompts({ memoryText, recentEvents, todoText, projectText, workingStateText })
...
assert(prompts.workingStateText === workingStateText, "buildPrompts must preserve provided Working State text")
...
assert(prompts.historyText.includes("Working state:"), "historyText must include Working State context")
...
assert(prompts.companionSystemPrompt.includes("What closes the current MVP?"), "companion prompt must include questions")
...
assert(!companionWithoutMemory.includes("<working_state_context>"), "empty Working State must not add context block")
...
const analysisContext = buildAnalysisContextText({
  memoryText,
  recentConversationText: historyOnly,
  todoText,
  projectText,
  workingStateText,
})
...
assert(analysisContext.includes("Working state:"), "analysis context must label Working State")
